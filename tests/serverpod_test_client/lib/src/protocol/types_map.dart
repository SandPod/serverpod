  ...
  factory TypesMap.fromJson(Map<String, dynamic> jsonSerialization) {
    return TypesMap(
      ...
      aDateTimeValue: (jsonSerialization['aDateTimeValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.DateTimeJsonExtension.fromJson(v)),
      ),
      aStringValue: (jsonSerialization['aStringValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, v as String),
      ),
      aByteDataValue: (jsonSerialization['aByteDataValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.ByteDataJsonExtension.fromJson(v)),
      ),
      aDurationValue: (jsonSerialization['aDurationValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.DurationJsonExtension.fromJson(v)),
      ),
      aUuidValue: (jsonSerialization['aUuidValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.UuidValueJsonExtension.fromJson(v)),
      ),
      aUriValue: (jsonSerialization['aUriValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.UriJsonExtension.fromJson(v)),
      ),
      aBigIntValue: (jsonSerialization['aBigIntValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i1.BigIntJsonExtension.fromJson(v)),
      ),
      anEnumValue: (jsonSerialization['anEnumValue'] as Map?)?.map(
        (k, v) => MapEntry(k as String, _i3.TestEnum.fromJson((v as int))),
      ),
  ...
  TypesMap copyWith({
    ...
    return TypesMap(
      anIntKey: anIntKey is Map<int, String>?
          ? anIntKey
          : this.anIntKey?.map((key0, value0) => MapEntry(key0, value0)),
      aBoolKey: aBoolKey is Map<bool, String>?
          ? aBoolKey
          : this.aBoolKey?.map((key0, value0) => MapEntry(key0, value0)),
      aDoubleKey: aDoubleKey is Map<double, String>?
          ? aDoubleKey
          : this.aDoubleKey?.map((key0, value0) => MapEntry(key0, value0)),
      aDateTimeKey: aDateTimeKey is Map<DateTime, String>?
          ? aDateTimeKey
          : this.aDateTimeKey?.map((key0, value0) => MapEntry(key0, value0)),
      aStringKey: aStringKey is Map<String, String>?
          ? aStringKey
          : this.aStringKey?.map((key0, value0) => MapEntry(key0, value0)),
      aByteDataKey: aByteDataKey is Map<_i2.ByteData, String>?
          ? aByteDataKey
          : this.aByteDataKey?.map(
              (key0, value0) => MapEntry(key0.clone(), value0),
            ),
      aDurationKey: aDurationKey is Map<Duration, String>?
          ? aDurationKey
          : this.aDurationKey?.map((key0, value0) => MapEntry(key0, value0)),
      aUuidKey: aUuidKey is Map<_i1.UuidValue, String>?
          ? aUuidKey
          : this.aUuidKey?.map((key0, value0) => MapEntry(key0, value0)),
      aUriKey: aUriKey is Map<Uri, String>?
          ? aUriKey
          : this.aUriKey?.map((key0, value0) => MapEntry(key0, value0)),
      aBigIntKey: aBigIntKey is Map<BigInt, String>?
          ? aBigIntKey
          : this.aBigIntKey?.map((key0, value0) => MapEntry(key0, value0)),
      anEnumKey: anEnumKey is Map<_i3.TestEnum, String>?
          ? anEnumKey
          : this.anEnumKey?.map((key0, value0) => MapEntry(key0, value0)),
      aStringifiedEnumKey:
      ...
      anObjectKey: anObjectKey is Map<_i5.Types, String>?
          ...
          : this.anObjectKey?.map(
              (key0, value0) => MapEntry(key0.copyWith(), value0),
            ),
      aMapKey: aMapKey is Map<Map<_i5.Types, String>, String>?
          ? aMapKey
          : this.aMapKey?.map(
              (key0, value0) => MapEntry(
                key0.map((key1, value1) => MapEntry(key1.copyWith(), value1)),
                value0,
              ),
            ),
      aListKey: aListKey is Map<List<_i5.Types>, String>?
          ...
          : this.aListKey?.map(
              (key0, value0) =>
                  MapEntry(key0.map((e1) => e1.copyWith()).toList(), value0),
            ),
      aRecordKey: aRecordKey is Map<(String,), String>?
          ? aRecordKey
          : this.aRecordKey?.map(
              (key0, value0) => MapEntry((key0.$1,), value0),
            ),
      anIntValue: anIntValue is Map<String, int>?
          ? anIntValue
          : this.anIntValue?.map((key0, value0) => MapEntry(key0, value0)),
      aBoolValue: aBoolValue is Map<String, bool>?
          ? aBoolValue
          : this.aBoolValue?.map((key0, value0) => MapEntry(key0, value0)),
      aDoubleValue: aDoubleValue is Map<String, double>?
          ? aDoubleValue
          : this.aDoubleValue?.map((key0, value0) => MapEntry(key0, value0)),
      aDateTimeValue: aDateTimeValue is Map<String, DateTime>?
          ? aDateTimeValue
          : this.aDateTimeValue?.map((key0, value0) => MapEntry(key0, value0)),
      aStringValue: aStringValue is Map<String, String>?
          ? aStringValue
          : this.aStringValue?.map((key0, value0) => MapEntry(key0, value0)),
      aByteDataValue: aByteDataValue is Map<String, _i2.ByteData>?
          ? aByteDataValue
          : this.aByteDataValue?.map(
              (key0, value0) => MapEntry(key0, value0.clone()),
            ),
      aDurationValue: aDurationValue is Map<String, Duration>?
          ? aDurationValue
          : this.aDurationValue?.map((key0, value0) => MapEntry(key0, value0)),
      aUuidValue: aUuidValue is Map<String, _i1.UuidValue>?
          ? aUuidValue
          : this.aUuidValue?.map((key0, value0) => MapEntry(key0, value0)),
      aUriValue: aUriValue is Map<String, Uri>?
          ? aUriValue
          : this.aUriValue?.map((key0, value0) => MapEntry(key0, value0)),
      aBigIntValue: aBigIntValue is Map<String, BigInt>?
          ? aBigIntValue
          : this.aBigIntValue?.map((key0, value0) => MapEntry(key0, value0)),
      anEnumValue: anEnumValue is Map<String, _i3.TestEnum>?
          ? anEnumValue
          : this.anEnumValue?.map((key0, value0) => MapEntry(key0, value0)),
      aStringifiedEnumValue:
      ...
      aMapValue: aMapValue is Map<String, Map<String, _i5.Types>>?
          ...
          : this.aMapValue?.map(
              (key0, value0) => MapEntry(
                key0,
                value0.map((key1, value1) => MapEntry(key1, value1.copyWith())),
              ),
            ),
