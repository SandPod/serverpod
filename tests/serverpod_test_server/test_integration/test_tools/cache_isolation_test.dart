    ...
    (final sessionBuilder, final endpoints) {
      group('when tests store data in local cache', () {
        test('then the value is retrieved.', () async {
          ...
          final cached1 = await endpoints.testTools.getFromLocalCache(
            sessionBuilder,
            'test-key',
          );
        ...
        test('then local cache should be empty from previous test', () async {
          ...
          final cached = await endpoints.testTools.getFromLocalCache(
            sessionBuilder,
            'test-key',
          );
          expect(
      ...
      group('when tests store data in localPrio cache', () {
        test('then the value is retrieved.', () async {
          ...
          await endpoints.testTools.putInLocalPrioCache(
            sessionBuilder,
            'prio-key',
            testData,
          );
          ...
          // Verify data was stored
          final cached1 = await endpoints.testTools.getFromLocalPrioCache(
            sessionBuilder,
            'prio-key',
          );
          expect(cached1, isNotNull);
        ...
        test(
          'then localPrio cache should be empty from previous test',
          () async {
            // This test should find cache empty due to automatic cleanup
            final cached = await endpoints.testTools.getFromLocalPrioCache(
              sessionBuilder,
              'prio-key',
            );
            expect(
      ...
      group('when tests store data in query cache', () {
        test('then the value is retrieved.', () async {
          ...
          await endpoints.testTools.putInQueryCache(
            sessionBuilder,
            'query-key',
            testData,
          );
          ...
          // Verify data was stored
          final cached1 = await endpoints.testTools.getFromQueryCache(
            sessionBuilder,
            'query-key',
          );
          expect(cached1, isNotNull);
        ...
        test('then query cache should be empty from previous test', () async {
          // This test should find cache empty due to automatic cleanup
          final cached = await endpoints.testTools.getFromQueryCache(
            sessionBuilder,
            'query-key',
          );
          expect(
      ...
      group('when tests store data with cache groups', () {
        test('then the value is retrieved.', () async {
          ...
          final cached2 = await endpoints.testTools.getFromLocalCache(
            sessionBuilder,
            'group-key-2',
          );
      ...
      group('when tests store data with different keys', () {
        test('then the value is retrieved.', () async {
          ...
          final cached3 = await endpoints.testTools.getFromLocalCache(
            sessionBuilder,
            'multi-3',
          );
      ...
      group('when cache data is set in setUp', () {
        ...
        test(
          'then setup cache data should be available in first test',
          () async {
            final cached = await endpoints.testTools.getFromLocalCache(
              sessionBuilder,
              'setup-key',
        ...
        test(
          'then setup cache data should still be available in second test',
          () async {
            final cached = await endpoints.testTools.getFromLocalCache(
              sessionBuilder,
              'setup-key',
