...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final retrieved = await cache.get<SimpleData>(key);
  ...
  test(
    ...
    () async {
      final retrieved = await cache.get<SimpleData>('missing');
  ...
  test(
    ...
    () async {
      const numEntries = cacheMaxSize * 2;

      for (var i = 0; i < numEntries; i++) {
        final entry = SimpleData(num: i);
        await cache.put('entry:$i', entry);
      }

      expect(cache.localSize, equals(cacheMaxSize));

      final first = await cache.get<SimpleData>('entry:0');
      expect(first, isNull);

      final last = await cache.get<SimpleData>('entry:${numEntries - 1}');
  ...
  test(
    ...
    () async {
      const key = 'value_to_be_computed';
